...
docker build -t nifi-build dev-support/docker
...
docker build -t "nifi-build-${USER_ID}" - <<UserSpecificDocker
FROM nifi-build
...
docker run --rm=true -t -i \
  -v "${PWD}:/home/${USER_NAME}/nifi${V_OPTS:-}" \
  -w "/home/${USER_NAME}/nifi" \
  -v "/srv/.m2:/home/${USER_NAME}/.m2${V_OPTS:-}" \
  -u "${USER_NAME}" \
  "nifi-build-${USER_ID}"
